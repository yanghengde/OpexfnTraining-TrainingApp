...
(function (window, $) {
    ...
    angular.module('ngGrid.services').factory('$domUtilityService', ['$utilityService', '$window', function ($utils, $window) {
        ...
        domUtilityService.BuildStyles = function ($scope, grid, digest) {
            var rowHeight = grid.config.rowHeight,
                ...
                sumWidth = 0,
                PADDING = 8,
                ngRowWidth, headerHeight, isScrollBarVisible, contentWidth;

            headerHeight = grid.$topPanel && grid.$topPanel.outerHeight() > 0 ? grid.$topPanel.outerHeight() : grid.config.headerRowHeight;
            isScrollBarVisible = (grid.maxCanvasHt >= ((grid.elementDims.rootMaxH - headerHeight) - domUtilityService.ScrollH)) ? true : false;
            var trw = $scope.totalRowWidth();
            contentWidth = grid.elementDims.rootMaxW - domUtilityService.ScrollW;

            if (isScrollBarVisible && trw === grid.elementDims.rootMaxW) {
                trw = trw - domUtilityService.ScrollW;
                ngRowWidth = trw;
            } else if (isScrollBarVisible) {
                ngRowWidth = trw < contentWidth ? contentWidth : trw;
            } else {
                ngRowWidth = grid.elementDims.rootMaxW;
            }

            css = "." + gridId + " .ngCanvas { width: " + trw + "px; }" +
                "." + gridId + " .ngRow { width: " + ngRowWidth + "px; }" +
                "." + gridId + " .ngCanvas { width: " + trw + "px; }" +
                "." + gridId + " .ngHeaderScroller { width: " + (trw + domUtilityService.ScrollH) + "px}";
            for (var i = 0; i < cols.length; i++) {
                var col = cols[i];
                var lastVisibleColumnIndex = _.findLastIndex(cols, function (col) {
                    return col.visible === true;
                });
                if (col.visible !== false) {
                    var rightPad = 0;
                    if ((i === lastVisibleColumnIndex) && (sumWidth + col.width <= grid.elementDims.rootMaxW)) {
                        if (isScrollBarVisible) {
                            rightPad = grid.elementDims.rootMaxW - sumWidth - col.width - domUtilityService.ScrollW;
                        } else {
                    ...
                    css += "." + gridId + " .col" + i + " { width: " + (col.width + rightPad) + "px; left: " + sumWidth + "px; height: " + rowHeight + "px }" +
                        ...
                        "." + gridId + " .ngCellText.colt" + i + "{ line-height: " + (rowHeight - PADDING) + "px; }";
                    sumWidth += col.width;
                }
            }
    ...
    var ngColumn = function (config, $scope, grid, domUtilityService, $templateCache, $utils) {
        ...
        self.gripOnMouseDown = function (event) {
            ...
            event.target.parentElement && (event.target.parentElement.style.cursor = 'col-resize')
            self.startMousePosition = event.clientX;
            self.origWidth = self.width;
            grid.dragDropService(document);
        ...
        self.gripOnMouseUp = function (event) {
            ...
            event.target.parentElement && (event.target.parentElement.style.cursor = 'default')
            domUtilityService.digest($scope);
    ...
    var ngGrid = function ($scope, options, sortService, domUtilityService, $filter, $templateCache, $utils, $timeout, $parse, $http, $q, dragDropManager) {
        var defaults = {
        ...
        self.filteredRows = [];
        self.dragDropService = dragDropManager.dragDrop;
    ...
    var ngRowFactory = function (grid, $scope, domUtilityService, $templateCache, $utils) {
        ...
        self.renderedChange = function () {
            ...
            if (grid.config.groups.length > 0 && grid.rowFactory.aggCache.length > 0 && !grid.isScrolled) {
                var isCollapsed = _.findIndex(grid.rowFactory.aggCache, { collapsed: false })
                if (isCollapsed !== -1) {
                    grid.maxCanvasHt = grid.calcMaxCanvasHeight();
                    domUtilityService.BuildStyles($scope, grid);
                }
            }
    ...
    ngGridDirectives.directive('ngGrid', ['$compile', '$filter', '$templateCache', '$sortService', '$domUtilityService', '$utilityService', '$timeout', '$parse', '$http', '$q', 'common.services.dragdrop.service', function ($compile, $filter, $templateCache, sortService, domUtilityService, $utils, $timeout, $parse, $http, $q, dragDropManager) {
        var ngGridDirective = {
            scope: true,
            compile: function () {
                return {
                    pre: function ($scope, iElement, iAttrs) {
                        ...
                        var grid = new ngGrid($scope, options, sortService, domUtilityService, $filter, $templateCache, $utils, $timeout, $parse, $http, $q, dragDropManager);
    ...
    angular.module('ngGrid').run(['$templateCache', function ($templateCache) {
        ...
        $templateCache.put('cellTemplate.html',
          "<div id=\"data-cell-container\" ng-attr-title=\"{{ COL_FIELD CUSTOM_FILTERS }}\"  class=\"ngCellText\" ng-class=\"col.colIndex()\"><span id=\"text-container\" ng-cell-text>{{COL_FIELD CUSTOM_FILTERS}}</span></div>"
        );
        ...
        $templateCache.put('headerCellTemplate.html',
          ...
          "\n" +
          "    <div id=\"header-sort-button\" ng-attr-title=\"{{col.displayName}}\" ng-click=\"col.sort($event)\" ng-class=\"'colt' + col.index\" class=\"ngHeaderText\">{{col.displayName}}</div>\r" +
          "\n" +
        ...
        $templateCache.put('rowTemplate.html',
          ...
          "\t<div ng-cell style=\"white-space:nowrap\" ng-style=\"{height: rowHeight}\"></div>\r" +
          "\n" +
          "</div>"
        );
